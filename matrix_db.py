...
cur = con.cursor()
# cur.execute("DROP TABLE matrixtest")
...
def insert_db(i, data):
    cur.execute("CREATE TABLE IF NOT EXISTS matrixtest (id INTEGER PRIMARY KEY, array BLOB)")
    cur.execute("INSERT INTO matrixtest VALUES (?,?)", (i, json.dumps(data.tolist()) ) )
    con.commit()
...
def Matrix_Generator(stemmed_page_list, page_id_list, chunk = 100):
    ...
    for i in range(0,4000):
        ...
        if chunk*(i+1)>len(Matrix):
            insert_db( i, Matrix[i*chunk:len(Matrix),:] )
        else:
            insert_db( i, Matrix[i*chunk:chunk*(i+1),:])
